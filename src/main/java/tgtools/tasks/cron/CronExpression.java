...
/**
 * 改代码引用自 xxl-job-admin中 由于封装问题 就拿出来包含到框架中了
 * 主要用于 解析 cron
 * <p>
...
public final class CronExpression implements Serializable, Cloneable {
    ...
    public static CronExpression create(String pCronExpression) throws ParseException {
        return new CronExpression(pCronExpression);
    }
    ...
    protected int storeExpressionVals(int pos, String s, int type)
        ...
        if (c == '*' || c == '/') {
            ...
            if (type == DAY_OF_MONTH && s.length() > i) {
                c = s.charAt(i);
                if (c == '-') {
                    TeamValueSet vs = getValue(0, s, i + 1);
            ...
            if (i >= s.length()) {
                ...
                c = s.charAt(i);
                if (c >= '0' && c <= '9') {
                    TeamValueSet vs = getValue(val, s, i);
                    val = vs.value;
                    i = vs.pos;
                }
    ...
    protected int checkNext(int pos, String s, int val, int type)
        ...
        if (c == '-') {
            ...
            c = s.charAt(i);
            if (c >= '0' && c <= '9') {
                TeamValueSet vs = getValue(v, s, i);
                end = vs.value;
                i = vs.pos;
            }
            if (i < s.length() && ((c = s.charAt(i)) == '/')) {
                ...
                c = s.charAt(i);
                if (c >= '0' && c <= '9') {
                    TeamValueSet vs = getValue(v2, s, i);
                    int v3 = vs.value;
    ...
    protected TeamValueSet getValue(int v, String s, int i) {
        char c = s.charAt(i);
        ...
        TeamValueSet val = new TeamValueSet();
